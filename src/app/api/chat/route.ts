...
async function getMLResponse(message: string): Promise<string> {
  ...
  if (lower.includes('rate') || lower.includes('freight') || lower.includes('cost') || lower.includes('price') || lower.includes('benchmark')) {
    ...
    if (mentioned.length >= 2) {
      ...
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000'}/api/predict/freight-rate`, {
          method: 'POST',
  ...
  if (lower.includes('delay') || lower.includes('late') || lower.includes('on time') || lower.includes('risk') || lower.includes('predict')) {
    ...
    if (matchedRoute) {
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000'}/api/predict/delay`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
